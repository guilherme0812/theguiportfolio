...
<?php 
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        $email  = strtolower( $_POST["email"] ); //Recebendo email do formulario
        ...
        $q_login = "SELECT * ";
        $q_login .= "FROM usuarios ";
        $q_login .= "WHERE email = '{$email}' and senha = '{$senha}' ";
...
<body>
    ...
    <section class="cadastro background-pizza pl-2 pr-2 pt-5 pb-5">
        <div class="container pb-5">
            <div class="row justify-content-md-center">
                <div class="col-md-10 col-lg-7 pb-5 mt-5 bg-white rounded opacity">
                    <form action="<?php echo $_SERVER["PHP_SELF"]; ?>" method="POST" class="mt-3">

                        <h1 class=" display-4 text-center">Login</h1>
                        <p class="paragrafo text-center text-muted mb-5">Faça seu cadastro e peça sua pizza ;)</p>

                        <div class="form-group mb-5">
                            <span class="text-danger"><?php if(isset($mensagem_de_erro) ){ echo $mensagem_de_erro ;} ?></span>
                            <input type="email" name="email" class="form-control mb-5" placeholder="Digite aqui seu E-mail" required onmousemove="recolherNumero(-1)">
                        ...
                        <span class="text-center text-muted d-block p-1">OU</span>
                        <a href="../cadastro/index.php"><button type="button" name="enviar" id="botaoDeEnviar" class="btn btn-warning btn-block">Criar meu cadastro</button></a>
                    </form>
                </div>
            </div>
        </div>
